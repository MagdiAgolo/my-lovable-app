...
export const linearService = {
  ...
  // Specialized method for debugging issues with specific engineers
  async getFatmaIssuesForSprint19(teamId: string): Promise<LinearIssue[]> {
    try {
      console.log('Using specialized method to fetch Fatma issues for Sprint 19');
      
      // Try multiple variations of Fatma's name to find matches
      const searchVariations = [
        "{ assignee: { name: { contains: \"Fatma\" } } }",
        "{ assignee: { name: { eq: \"Fatma Akram\" } } }",
        "{ assignee: { name: { eq: \"fatma akram\" } } }"
      ];
      ...
      // Try each variation
      for (const filterVariation of searchVariations) {
        console.log(`Trying filter variation: ${filterVariation}`);
        
        // Using a simple query that directly looks for Fatma's assigned issues
        const query = `
          query FatmaIssuesSprint19($teamId: String!) {
            team(id: $teamId) {
              id
              name
              key
        ...
        if (response?.team?.issues?.nodes && response.team.issues.nodes.length > 0) {
          console.log(`Found ${response.team.issues.nodes.length} issues with filter: ${filterVariation}`);
          allIssues = [...allIssues, ...response.team.issues.nodes];
        } else {
          console.log(`No issues found with filter: ${filterVariation}`);
        }
      }
      
      // De-duplicate issues by ID
      const uniqueIssues = Array.from(new Map(allIssues.map(item => [item.id, item])).values());
      console.log(`Found ${uniqueIssues.length} unique issues for Fatma after trying all variations`);
      
      if (uniqueIssues.length === 0) {
        console.warn('No issues found for Fatma with any variation');
        
        // As a fallback, get all issues and filter on the client side
        console.log('Trying fallback: get all issues and filter for Fatma client-side');
        const allTeamIssuesQuery = `
          query AllIssues($teamId: String!) {
            team(id: $teamId) {
              id
              name
              key
        ...
        const allIssuesResponse = await this.executeGraphQL(allTeamIssuesQuery, { teamId });
        
        if (allIssuesResponse?.team?.issues?.nodes) {
          // Log all assignees for debugging
          const allAssignees = new Set<string>();
          allIssuesResponse.team.issues.nodes.forEach((issue: any) => {
            if (issue.assignee?.name) {
              allAssignees.add(issue.assignee.name);
            }
          });
          console.log('All assignee names in team:', Array.from(allAssignees));
          
          // Client-side filter for anything containing "fatma" case-insensitive
          const fatmaIssues = allIssuesResponse.team.issues.nodes.filter((issue: any) => 
            issue.assignee?.name && issue.assignee.name.toLowerCase().includes('fatma')
          );
          
          if (fatmaIssues.length > 0) {
            console.log(`Found ${fatmaIssues.length} issues by client-side filtering for name containing "fatma"`);
            uniqueIssues.push(...fatmaIssues);
          } else {
            console.warn('No issues found for Fatma even with client-side filtering');
          }
        }
      }
      ...
      // Process the issues
      const issues = uniqueIssues.map((issue: any) => {
        const isCompleted = this.isIssueCompleted(issue);
        const velocity = isCompleted ? issue.estimate || 0 : 0;
      ...
      // Further filter to the specific sprint if needed - Sprint 19
      const sprint19Issues = issues.filter((issue: LinearIssue) => {
        return issue.cycle?.name?.includes('19');
      });
      
      console.log(`Found ${sprint19Issues.length} issues for Fatma in Sprint 19 (out of ${issues.length} total Fatma issues)`);
      return sprint19Issues;
      
    } catch (error) {
      console.error('Error fetching Fatma issues:', error);
      return [];
    }
  }
};